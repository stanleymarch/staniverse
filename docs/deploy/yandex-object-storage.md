...
Бинарник 8th Wall в Object Storage не публикуется. iPhone/iPad без нативного
WebXR после явного нажатия «AR через камеру» загружает точно закреплённую версию
`@8thwall/engine-binary@1.0.0` с jsDelivr. Это официальный способ подключения
из документации пакета.

Страница не содержит preload или статический `<script>` движка: адаптер создаёт
его только внутри обработчика кнопки, затем отдельно вызывает
`XR8.loadChunk('slam')`. Поэтому обычные посетители сайта и поисковые роботы не
запрашивают XR, а core и SLAM обслуживаются CDN, не нашим бакетом. Полный
дистрибутив с Face Effects и semantic-моделями также не попадает в production.

Цена решения — внешняя runtime-зависимость: при недоступности jsDelivr адаптер
по таймауту возвращает пользователя в экранный режим. URL содержит точную
версию, поэтому обновление движка возможно только явным изменением кода и
повторной проверкой на физическом iPhone/iPad.
