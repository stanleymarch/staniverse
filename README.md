...
`npm run build` — это `astro check`, затем `astro build`, затем `postbuild`: он рисует 15 брендовых OG-карточек в `dist/og/` и пересобирает четыре woff2 в `dist/fonts/` под символы, которые встречаются в этой сборке. Мастера в `public/fonts/` остаются полными, поэтому `npm run dev` отдаёт шрифты целиком, а новый пост с новым символом сам расширяет сабсет — руками ничего регенерировать не нужно.
