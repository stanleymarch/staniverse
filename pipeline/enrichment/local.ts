...
const generatedAt = archive.publications.map((publication) => publication.editedDate ?? publication.date).filter((value): value is string => Boolean(value)).sort().at(-1) ?? new Date(0).toISOString();
const bundle: EnrichmentBundle = {
  version: 1,
  generatedAt,
  results: archive.publications.map((publication) => enrichLocally(publication, knownTargets)),
};
