...
// re-running the whole ffmpeg pipeline, so carry them across. Primary key is
// message + file, but a Telegram re-export renames media files (the timestamp in
// the name follows the export date), so a message with exactly one media file on
// both sides also carries by message id alone — otherwise every re-download
// would orphan the already published webp/mp4 set.
const publications=normalizeExport(merged,handle);let carriedMediaPaths=0;const canonicalPath=resolve("pipeline/telegram/archive/canonical.json");try{const previousCanonical=JSON.parse(await readFile(canonicalPath,"utf8"))as{publications?:{media?:{messageId:number;sourcePath:string;publicPath?:string}[]}[]};const knownMedia=new Map<string,string>();const previousByMessage=new Map<number,{publicPath:string;count:number}>();for(const publication of previousCanonical.publications??[])for(const media of publication.media??[])if(media.publicPath){knownMedia.set(`${media.messageId}|${media.sourcePath}`,media.publicPath);const aggregate=previousByMessage.get(media.messageId)??{publicPath:media.publicPath,count:0};aggregate.count+=1;previousByMessage.set(media.messageId,aggregate)}const currentByMessage=new Map<number,number>();for(const publication of publications)for(const media of publication.media)currentByMessage.set(media.messageId,(currentByMessage.get(media.messageId)??0)+1);for(const publication of publications)for(const media of publication.media){const carried=knownMedia.get(`${media.messageId}|${media.sourcePath}`)??(currentByMessage.get(media.messageId)===1&&previousByMessage.get(media.messageId)?.count===1?previousByMessage.get(media.messageId)!.publicPath:undefined);if(carried){media.publicPath=carried;carriedMediaPaths++}}}catch{}
