...
for(const publication of archive.publications)for(const media of publication.media){
  const stem=`${publication.sourceId}-${media.messageId}-${media.order}`;const source=resolve(sourceFolder,media.sourcePath);
  try{
    sourceBytes+=(await stat(source)).size;
    let basename:string;
    if(media.type==="image"){
      basename=`${stem}.webp`;await sharp(source).rotate().resize({width:1600,height:1600,fit:"inside",withoutEnlargement:true}).webp({quality:82,effort:4}).toFile(resolve(output,basename));
    }else if(media.type==="video"){
      basename=`${stem}.mp4`;await run("ffmpeg",["-loglevel","error","-y","-i",source,"-vf","scale=w='min(960,iw)':h=-2","-c:v","libx264","-preset","medium","-crf","30","-c:a","aac","-b:a","96k","-movflags","+faststart",resolve(output,basename)]);
    }else if(media.type==="audio"){
      basename=`${stem}.mp3`;await run("ffmpeg",["-loglevel","error","-y","-i",source,"-c:a","libmp3lame","-b:a","96k",resolve(output,basename)]);
    }else{
      basename=`${stem}${extname(media.sourcePath).toLowerCase()||".bin"}`;await copyFile(source,resolve(output,basename));
    }
    outputBytes+=(await stat(resolve(output,basename))).size;media.publicPath=`/media/telegram/${basename}`;written++;
